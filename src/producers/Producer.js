...
class Producer {
    ...
    */
    async sendToTopic(datasets, sender) {
        ...
        console.log(`${moment.utc().format(consts.dateTime.bigqueryZonelessTimestampFormat)}, ${messages.length} messages [${topicName}:${result[0].baseOffset}-${Number(result[0].baseOffset) + (messages.length - 1)}, sender:${this.sender}]`)
